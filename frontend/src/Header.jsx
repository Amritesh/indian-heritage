...
export default function Header() {
  ...
  return (
    <header style={{background: '#fff', borderBottom: '1px solid #FEDC85', position: 'relative'}}>
      <div className="container-custom" style={{height: 92, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 24}}>
        <div style={{display: 'flex', alignItems: 'center', gap: 16}}>
          <img src="https://api.builder.io/api/v1/image/assets/TEMP/60e679c2382ff8daadccb9add703b2d7d9d98f0f?width=128" alt="logo" style={{width:48,height:48}} />
          <div style={{fontFamily: 'Alegreya Sans, sans-serif', color: primary, fontWeight: 600, fontSize: 18}}>Indian Heritage.</div>
        </div>

        <div style={{flex: 1, display: 'flex', justifyContent: 'center'}}>
          <div style={{maxWidth: 520, width: '100%', display: 'flex', alignItems: 'center', padding: '12px 16px', background: '#fff', borderRadius: 6, boxShadow: '0 2px 6px rgba(51,37,2,0.06)'}}>
        ...
        <div style={{display: 'flex', alignItems: 'center', gap: 16}}>
          <div style={{position: 'relative'}}>
            ...
            <span style={{position:'absolute', right: -4, top: -4, width:8, height:8, borderRadius: 8, background: '#BA2525', display:'inline-block'}} />
          </div>
          <img src="/assets/header-profile.png" alt="profile" style={{width:40,height:40,borderRadius:4,boxShadow:'0 2px 4px rgba(51,19,0,0.12)'}} />
        </div>
      </div>
    </header>
  );
